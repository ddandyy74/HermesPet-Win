...
| ID | 任务 | 状态 | 备注 |
|----|------|------|------|
| M3.1.1 | 会话列表侧边栏 UI | ✅ | ConversationListControl.xaml |
| M3.1.2 | 新建/切换/删除会话功能 | ✅ | DeleteConversation 方法 + 确认对话框 |
...
### M3.3 连接状态+模型列表 ✅ 已完成（2026-06-07）

| ID | 任务 | 状态 | 备注 |
|----|------|------|------|
| M3.3.1 | ConnectionStatus 检测 | ✅ | ChatViewModel.CheckConnectionAsync，5秒超时 |
| M3.3.2 | IslandViewModel 集成连接状态 | ✅ | ObservableProperty + 动态岛同步 |
| M3.3.3 | 动态岛连接状态 UI | ✅ | 右上角小圆点（绿/黄/灰/红） |
| M3.3.4 | FetchModelsAsync 模型列表 | ✅ | OpenAICompatibleClient 实现，10秒超时 |
| M3.3.5 | 连接失败错误提示 | ✅ | ErrorMessage 显示友好消息 |
...
### M4 验收检查清单 ✅ 已完成（2026-06-07）

#### M4.0 宠物动画移植
- [x] 5 种宠物动画全部移植完成，像素级还原 macOS 版本
- [x] 宠物动画流畅度 ≥ 30fps，空闲时可降至 12fps
- [x] 支持动态调色（实时切换配色方案）
- [x] 内存占用 < 5MB

#### M4.1 语音输入
- [x] 按住说话功能正常（Ctrl+Shift+V 切换模式）
- [x] 音量可视化正常（RMS 归一化 + ProgressBar）
- [x] Whisper.NET 本地识别集成
- [x] 模型自动下载（首次使用）

#### M4.2 截图功能
- [x] 截图可捕获并发送给 AI（全屏/窗口/区域）
- [x] 多模态支持（文本 + 图片）
- [x] 不支持图片的 AI 模式给出提示

#### M4.3 快速询问+置顶卡片+知识图谱
- [x] 快速询问浮窗正常（Ctrl+Shift+Space）
- [x] 置顶卡片功能正常（Ctrl+Shift+P）
- [x] 知识图谱功能正常（Ctrl+Shift+G）
- [x] 选中文本读取正常
...
#### 已知限制
- ⚠️ Windows.Graphics.Capture 部分为 TODO（当前使用 BitBlt）
- ⚠️ M4.4 任务卡片+每日简报未实现
...
|------|--------|--------|----------|--------|
| 2026-06-07 | M4 最终验收 | M4 高级功能阶段 QA 验证通过（24 个文件，17 项验收标准）| 2 个非阻塞警告（CS4014/CS0414）| 进入 M5 打磨发布阶段 |
